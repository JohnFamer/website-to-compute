...
const PAYMENT_PROVIDER = "โอนเงินผ่านบัญชีร้าน / QR Payment";

function formatBaht(value) {
  return `${Math.round(Number(value || 0)).toLocaleString()} บาท`;
}
...
async function createThaiPaymentRequest(payload) {
  const paymentId = `DQ-PAY-${Date.now()}`;
  return {
    paymentId,
    status: "payment_created",
    checkoutUrl: "#thai-payment-checkout",
    paymentNote: `โอนเงิน ${formatBaht(payload.amountThb)} เข้าบัญชีร้าน Disk Quest และแนบสลิปด้วยรหัส ${paymentId}`,
    demo: true,
  };
}
...
const navItems = [
  ...
  { icon: "coins", label: "เปิดดีลเงินบาท" },
  { icon: "gamepad", label: "คลังดีล" },
...
const releases = [
  { id: "DQ-THB-001", game: "Zelda TOTK", receiver: "Tan", sender: "Mild K.", paidThb: 1790, releaseThb: 1790 - SERVICE_FEE_THB, status: "รอปล่อยเงิน" },
  { id: "DQ-THB-002", game: "Spider-Man 2", receiver: "Arm", sender: "Krit T.", paidThb: 1290, releaseThb: 1290 - SERVICE_FEE_THB, status: "จัดส่งแล้ว" },
  { id: "DQ-THB-003", game: "God of War Ragnarök", receiver: "View", sender: "Narin P.", paidThb: 1890, releaseThb: 1890 - SERVICE_FEE_THB, status: "ชำระแล้ว" },
];

const users = [
  { name: "Narin P.", tier: "Pro Trader", items: 12, completed: 8, releaseThb: 18600 },
  { name: "Beam S.", tier: "Verified", items: 6, completed: 3, releaseThb: 7400 },
  { name: "Mild K.", tier: "Collector", items: 9, completed: 7, releaseThb: 12890 },
  { name: "Krit T.", tier: "New", items: 3, completed: 1, releaseThb: 890 },
];
...
function runSelfTests() {
  console.assert(initialItems.length === 4, "Expected 4 initial Thai-baht exchange items");
  console.assert(initialItems.every((item) => Number.isFinite(item.priceThb)), "All initial items should have numeric THB prices");
  console.assert(platformData[3].ps5 === 58, "Expected April PS5 count to be 58");
  console.assert(releases[0].releaseThb === 1760, "Expected first release amount to subtract 30 baht fee");
  ...
  console.assert(formatBaht(3500) === "3,500 บาท", "Baht formatting should use separators");
  console.assert(formatFee() === "30 บาท / ดีล", "Fee text should be 30 baht per deal");
  console.assert(PAYMENT_PROVIDER === "โอนเงินผ่านบัญชีร้าน / QR Payment", "Payment provider should be Thai baht transfer flow");
    ...
    <header style={styles.topbar}>
      <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
        ...
        <div>
          ...
          <p style={styles.subtitle}>Disk Quest ระบบซื้อขายแผ่นเกมด้วยเงินบาท โดยร้านเป็นคนกลางป้องกันโกง</p>
        </div>
      </div>
...
function SummaryCards({ items }) {
  ...
  const totalEscrowThb = items.reduce((sum, item) => sum + Number(item.priceThb || 0), 0);
  const summary = [
    { label: "มูลค่าดีลที่ร้านดูแล", value: formatBaht(totalEscrowThb), helper: "ยอดซื้อขายเงินบาทในระบบ", tone: "green", icon: "shield" },
    { label: "เกมพร้อมขาย", value: `${ready} แผ่น`, helper: "พร้อมเปิดดีล", tone: "green", icon: "gamepad" },
  ...
  return (
    <section style={styles.heroWelcome}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", gap: 24, alignItems: "center" }}>
        <div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "7px 12px", borderRadius: 999, background: "rgba(255,255,255,0.14)", fontSize: 12, fontWeight: 900, marginBottom: 14 }}><SvgIcon name="shield" size={15} /> DISK QUEST SAFE TRADE</div>
          <h2 style={{ margin: 0, fontSize: 36, lineHeight: 1.08, letterSpacing: -1.2 }}>ซื้อขายแผ่นเกมมือ 1 / มือ 2 ด้วยเงินบาทผ่านร้านคนกลาง</h2>
          <div style={{ marginTop: 14, fontSize: 20, fontWeight: 900, color: "#FDE68A" }}>✔ ปลอดภัย 100% • มีคนกลาง • ไม่มีโกง</div>
          <p style={{ margin: "12px 0 0", color: "rgba(255,255,255,0.82)", maxWidth: 680, lineHeight: 1.7 }}>ผู้ซื้อโอนเงินบาทเข้าบัญชีร้านหรือ QR Payment ก่อน ผู้ขายส่งแผ่นเกม หลังผู้ซื้อยืนยันรับสินค้า ร้านจึงปล่อยเงินให้ผู้ขาย พร้อมค่าบริการ {formatFee()} ต่อรายการ</p>
        </div>
        <button type="button" style={{ border: 0, borderRadius: 16, padding: "15px 20px", background: "white", color: colors.green700, fontWeight: 950, cursor: "pointer", boxShadow: "0 14px 30px rgba(0,0,0,0.16)" }} onClick={() => setActivePage("เปิดดีลเงินบาท")}>+ เปิดดีลเงินบาทใหม่</button>
      </div>
    </section>
  );
}
  ...
  return (
    <div style={styles.chartCard}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22, gap: 12 }}>
        <h2 style={styles.panelTitle}>▦ จำนวนดีลซื้อขายแยกตามแพลตฟอร์ม</h2>
  ...
  return (
    <div style={{ ...styles.card, padding: 24, marginTop: 22 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, gap: 12 }}>
        <h2 style={styles.panelTitle}>ดีลซื้อขายล่าสุด</h2>
      ...
      {items.length === 0 ? <EmptyState title="ยังไม่มีดีลซื้อขาย" /> : items.map((item) => (
        <div key={item.code} style={{ display: "grid", gridTemplateColumns: "54px 1fr auto", gap: 14, alignItems: "center", padding: "13px 0", borderTop: `1px solid ${colors.border}` }}>
          ...
          <div>
            ...
            <div style={{ color: colors.muted, fontSize: 13, marginTop: 3 }}>รหัสสินค้า: {item.code} • {formatBaht(item.priceThb)}</div>
...
function TodayOverview({ items }) {
  ...
  const waiting = items.length - ready;
  const rows = [
    ["gamepad", "เกมพร้อมขาย", `${ready} แผ่น`, "green"],
    ["coins", "ดีลเงินบาทใหม่", "8 ดีล", "green"],
    ["checklist", "รอตรวจสภาพ", `${waiting} แผ่น`, "orange"],
    ["shield", "ค่าบริการคนกลาง", formatFee(), "red"],
  ];
  return (
...
function ContactPanel() {
  return <div style={{ ...styles.card, padding: 22, marginTop: 18 }}><h2 style={styles.panelTitle}>ช่องทางการติดต่อ</h2><div style={{ display: "grid", gap: 14, marginTop: 18 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ color: colors.green600, fontWeight: 950 }}>LINE</span><span style={{ color: colors.muted }}>@diskquest ›</span></div><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ color: colors.blue, fontWeight: 950 }}>ชำระเงิน</span><span style={{ color: colors.muted }}>บัญชีร้าน / QR Payment ›</span></div></div></div>;
}
...
function DashboardPage({ setActivePage, items }) {
  return (
    <>
      ...
      <button type="button" style={styles.fab} onClick={() => setActivePage("เปิดดีลเงินบาท")}>ดีลเงินบาทใหม่ <span style={{ fontSize: 24, lineHeight: 1 }}>+</span></button>
    </>
  );
}
...
function DepositPage({ onAddItem }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (required.some((value) => !String(value).trim())) {
      setMessage("กรุณากรอกข้อมูลให้ครบก่อนสร้างดีลเงินบาท");
      return;
    }
    const numericThb = Number(String(form.price).replace(/,/g, ""));
    if (!Number.isFinite(numericThb) || numericThb <= 0) {
      setMessage("กรุณากรอกราคาเงินบาทเป็นตัวเลขที่ถูกต้อง");
      return;
    }
    setIsSubmitting(true);
    setMessage("กำลังสร้างรายการชำระเงิน...");
    try {
      const createdPayment = await createThaiPaymentRequest({
        amountThb: Math.round(numericThb),
      ...
      onAddItem({ ...form, priceThb: Math.round(numericThb), payment: createdPayment });
      setMessage("สร้างดีลเงินบาทเรียบร้อยแล้ว");
      setForm({ title: "", platform: "", type: "", price: "", owner: "", phone: "" });
    } catch (error) {
      setMessage(`สร้างรายการชำระเงินไม่สำเร็จ: ${error.message}`);
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ ...styles.card, padding: 28 }}>
      <h2 style={{ marginTop: 0, fontSize: 30 }}>เปิดดีลเงินบาท</h2>
      <p style={{ color: colors.muted, lineHeight: 1.8 }}>ฟอร์มเปิดดีลซื้อขายแผ่นเกมด้วยเงินบาท ร้านเป็นคนกลางรับเงินผ่านบัญชีร้านหรือ QR Payment ก่อนปล่อยเงินให้ผู้ขายเมื่อผู้ซื้อยืนยันรับสินค้า</p>
      <div style={styles.formGrid}>
        ...
        <Field label="ราคาที่ต้องการขาย (บาท)" value={form.price} onChange={(value) => update("price", value)} placeholder="1290" type="text" />
        <Field label="ชื่อผู้ใช้บริการ" value={form.owner} onChange={(value) => update("owner", value)} placeholder="ชื่อผู้ขาย / ผู้เปิดดีล" />
        ...
        <div style={{ marginTop: 18, border: `1px solid ${colors.border}`, borderRadius: 18, padding: 18, background: "#F8FAFC" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <div>
              ...
              <div style={{ color: colors.muted, fontSize: 13, marginTop: 4 }}>Payment ID: {payment.paymentId}</div>
            </div>
            <StatusBadge tone="blue">Demo Payment</StatusBadge>
          </div>
          <textarea readOnly value={payment.paymentNote || ""} style={{ ...styles.input, minHeight: 92, marginTop: 14, fontFamily: "monospace", resize: "vertical" }} />
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
            <a href={payment.checkoutUrl || "#"} style={{ ...styles.primaryButton, textDecoration: "none", display: "inline-flex" }}>เปิดหน้าชำระเงิน</a>
            <span style={{ ...styles.chip, display: "inline-flex", alignItems: "center" }}>ค่าบริการ {formatFee()}</span>
          </div>
        </div>
      )}
      <button disabled={isSubmitting} type="submit" style={{ ...styles.primaryButton, marginTop: 20, opacity: isSubmitting ? 0.7 : 1 }}>{isSubmitting ? "กำลังสร้างดีล..." : "เปิดดีลเงินบาท +"}</button>
    </form>
  );
}

function InventoryPage({ items }) {
  return <div style={{ ...styles.card, padding: 24 }}><h2 style={{ marginTop: 0, fontSize: 30 }}>คลังดีล</h2>{items.length === 0 ? <EmptyState title="ยังไม่มีดีลซื้อขาย" /> : items.map((item) => <div key={item.code} style={{ display: "grid", gridTemplateColumns: "64px 1fr 120px 130px", gap: 14, alignItems: "center", padding: 14, border: `1px solid ${colors.border}`, borderRadius: 14, marginTop: 12 }}><img src={item.image} alt={item.title} style={{ width: 64, height: 70, borderRadius: 10, objectFit: "cover" }} /><div><div style={{ fontWeight: 950 }}>{item.title}</div><div style={{ color: colors.muted, fontSize: 13 }}>{item.code} • {item.platform} • {item.owner}</div></div><div style={{ fontWeight: 950, color: colors.green700 }}>{formatBaht(item.priceThb)}</div><StatusBadge tone={getStatusTone(item.status)}>{item.status}</StatusBadge></div>)}</div>;
}
...
function HelpPage() {
  return <div style={{ ...styles.card, padding: 28 }}><h2 style={{ marginTop: 0, fontSize: 30 }}>ช่วยเหลือ</h2>{["วิธีเปิดดีลซื้อขายด้วยเงินบาท", `ค่าบริการ ${formatFee()}`, "มาตรฐานการตรวจสภาพ", "ขั้นตอนการปล่อยเงินหลังยืนยันรับแผ่นเกม", "ติดต่อทีมงาน Disk Quest"].map((x) => <div key={x} style={{ padding: 16, borderTop: `1px solid ${colors.border}`, display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 850 }}>{x}</span><span style={{ color: colors.muted }}>›</span></div>)}</div>;
}
...
export default function DiskQuestApp() {
  ...
  const handleAddItem = (form) => {
    const priceThb = Number(form.priceThb || form.price || 0);
    const item = {
      title: form.title.trim(),
      platform: form.platform.trim(),
      type: form.type.trim(),
      ...
      priceThb,
      serviceFeeThb: SERVICE_FEE_THB,
      status: form.payment?.demo ? "รอชำระเงิน demo" : "รอชำระเงิน",
      paymentId: form.payment?.paymentId || "",
      checkoutUrl: form.payment?.checkoutUrl || "",
      grade: "รอตรวจ",
    ...
    setActivePage("คลังดีล");
  };

  const page = useMemo(() => {
    if (activePage === "หน้าหลัก") return <DashboardPage setActivePage={setActivePage} items={items} />;
    if (activePage === "เปิดดีลเงินบาท") return <DepositPage onAddItem={handleAddItem} />;
    if (activePage === "คลังดีล") return <InventoryPage items={items} />;
      ...
      <div style={styles.appShell}>
        <aside style={styles.sidebar}>
          <div style={styles.brand}>
            ...
            <div><div style={{ fontSize: 25, fontWeight: 950, letterSpacing: -1 }}>DISK QUEST</div><div style={{ fontSize: 11, letterSpacing: 1.1, fontWeight: 800, opacity: 0.82 }}>THAI BAHT GAME MARKETPLACE</div></div>
          </div>
          <nav>
            {navItems.map((item) => {
          ...
          <div style={styles.serviceCard}>
            ...
            {["ซื้อขายแผ่นเกมด้วยเงินบาท", "ร้านเป็นคนกลางรับเงิน", "🚨 โดนโกง = ร้านคืนเงิน", "ทีมงานมืออาชีพ"].map((text) => <div key={text} style={{ display: "flex", gap: 10, alignItems: "center", margin: "12px 0", color: "rgba(255,255,255,0.9)", fontWeight: 700 }}><span>✓</span>{text}</div>)}
